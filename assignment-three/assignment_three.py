...
def recursive_bubble(totals):
  """Sorting based on the assumption that only the last item needs to be 'bubbled up' to its correct space
...
def test_recursive_bubble():
    ...
    # Sort in place
    recursive_bubble(mock_totals) 

    assert(mock_totals == expected_totals)

test_recursive_bubble() 

def top_k_sort_k_recursive(totals, k):
  ...
  for total in totals:
    ...
    if total_val > bottom_k_val:
      top_k[0] = total
      recursive_bubble(top_k)

  return top_k

def test_top_k_sort_k_recursive():
  mock_county_totals = [("A", 0), ("B", 1), ("C", 2), ("D", 3), ("E", 4), ("F", 4)]
  expected_top_k = [("D", 3), ("F", 4), ("E", 4)]

  obs = top_k_sort_k_recursive(mock_county_totals, 3)

  assert(obs == expected_top_k)

test_top_k_sort_k_recursive()
...
"""
Compare the built-in and recursive sort algorithm approaches
...
start_time = time.perf_counter()
top_counties_rec = top_k_sort_k_recursive(county_totals, k)
stop_time = time.perf_counter()
print(f"top counties alt: {top_counties_rec}")
